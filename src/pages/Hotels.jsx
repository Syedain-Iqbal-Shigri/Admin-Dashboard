  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} viewBox="0 0 24 24" fill="currentColor" className={`w-3 h-3 ${i < count ? 'text-amber-400' : 'text-slate-200 dark:text-slate-600'}`}>
...
export default function Hotels() {
  const { data, loading, error, refetch } = useQuery(getHotels)
  ...
  const totalRooms = data.filter(h => h.status === 'Active').reduce((sum, h) => sum + h.rooms, 0)
  const ratedHotels = data.filter(h => h.rating > 0)
  const avgRating = ratedHotels.length
    ? (ratedHotels.reduce((sum, h) => sum + h.rating, 0) / ratedHotels.length).toFixed(1)
    : '—'

  const handleDelete = async (id) => {
    const ok = await confirm('Delete this hotel? This cannot be undone.', 'Delete Hotel')
    if (!ok) return
    try {
      ...
      toast('Failed to delete hotel. Please try again.', 'error')
    }
  }
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Hotels"
        description="Manage hotel listings and partner properties"
        action={<AddButton label="Add Hotel" />}
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MiniStat label="Total Hotels" value={data.length} color="text-blue-500" loading={loading} />
        <MiniStat label="Active" value={active} color="text-emerald-500" loading={loading} />
        <MiniStat label="Total Rooms" value={totalRooms.toLocaleString()} color="text-violet-500" loading={loading} />
        <MiniStat label="Avg Rating" value={avgRating === '—' ? avgRating : `${avgRating} ★`} color="text-amber-500" loading={loading} />
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700">
                {['ID', 'Hotel Name', 'City', 'Stars', 'Rating', 'Rooms', 'Owner', 'Status', 'Actions'].map(h => (
            ...
            {loading
              ...
              : (
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                  {filtered.length === 0
                    ? <EmptyState message="No hotels match your filters." colSpan={9} />
                    : filtered.map(h => (
                      <tr key={h.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                        <td className="px-4 py-3.5 font-mono text-[11px] text-slate-400 dark:text-slate-500">{h.id}</td>
                        <td className="px-4 py-3.5 font-medium text-slate-800 dark:text-slate-100 whitespace-nowrap">{h.name}</td>
                        <td className="px-4 py-3.5 text-slate-500 dark:text-slate-400">{h.city}</td>
                        <td className="px-4 py-3.5"><Stars count={h.stars} /></td>
                        <td className="px-4 py-3.5 font-medium text-slate-700 dark:text-slate-200">
                          {h.rating > 0 ? h.rating : <span className="text-slate-300 dark:text-slate-600">—</span>}
                        </td>
                        <td className="px-4 py-3.5 text-slate-600 dark:text-slate-300">{h.rooms}</td>
                        <td className="px-4 py-3.5 text-slate-500 dark:text-slate-400 whitespace-nowrap">{h.owner}</td>
                        <td className="px-4 py-3.5">
                          <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${statusStyles[h.status]}`}>{h.status}</span>
                        </td>
                        <td className="px-4 py-3.5">
        ...
        <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-700">
          <span className="text-[12px] text-slate-400 dark:text-slate-500">
            {loading ? 'Loading…' : `Showing ${filtered.length} of ${data.length} hotels`}
          </span>
        </div>
      </div>
    </div>
