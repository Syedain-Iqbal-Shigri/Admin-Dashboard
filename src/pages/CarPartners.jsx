...
import { getCarPartners, deleteCarPartner } from '../services/carPartners'
...
export default function CarPartners() {
  const { data, loading, error, refetch } = useQuery(getCarPartners)
  ...
  const totalCars = data.filter(p => p.status === 'Active').reduce((sum, p) => sum + p.cars, 0)
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Car Partners"
        description="Manage car rental partner relationships"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MiniStat label="Cars Listed" value={totalCars} color="text-violet-500" loading={loading} />
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700">
                {['ID', 'Partner', 'Contact', 'Phone', 'Cars', 'Commission', 'City', 'Joined', 'Status', 'Actions'].map(h => (
                  ...
                  {filtered.length === 0
                    ...
                    : filtered.map(p => (
                      <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="px-4 py-3.5 font-semibold text-slate-700 dark:text-slate-200 text-center">{p.cars}</td>
